...
module.exports = async (req, res) => {
  try {
    ...
    const mongoData = await PriceList.find({}).lean();
    ...
    console.log('No data found in MongoDB, fetching from ServiceNow');
    ...
    if (serviceNowData && serviceNowData.length > 0) {
      ...
      const savePromises = serviceNowData.map(async (item) => {
        try {
          ...
          const savedDoc = await priceList.save();
          // Add MongoDB ID to the response
          item._id = savedDoc._id.toString();
          item.mongoId = savedDoc._id.toString();
          return item;
        } catch (error) {
          console.error(`Error saving price list ${item.sys_id} to MongoDB:`, error);
